...
interface AirportDAO {
    ...
    @Query("SELECT iata_code,name FROM airport WHERE iata_code LIKE '%' || :searchTerm || '%' OR name LIKE '%' || :searchTerm || '%' ORDER BY passengers DESC")
     ...
     */
    @Query("SELECT iata_code,name FROM airport WHERE iata_code!=:iataCode ORDER BY passengers DESC ")
